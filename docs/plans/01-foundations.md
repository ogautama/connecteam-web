...
- `src/components/layouts/MemberLayout.tsx` — sidebar nav shell (static
  placeholder links for now — real gating comes in PR-06), header with
...
- Vitest + React Testing Library setup (`vitest.config.ts`, `npm test`
  script), since every later PR needs a place to put unit tests.

## Out of scope

Auth, database, real page content — all later PRs.

## Independence notes

Nothing to stub — this PR has no external dependencies.
