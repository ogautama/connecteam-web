...
import { PENGUNDANG_UNIT_OPTIONS } from "@/lib/memberIntakeOptions";
import { setItemCompletion } from "@/lib/onboardingProgress";

/**
 * "Isi Data" — the personal-data intake form (Plan 07), fields copied from
 * the real Google Form it replaces. Submitting it also marks the
 * "join-isi-data" checklist item done on the Onboarding hub, so there's no
 * separate manual checkbox step once the real data is saved. Photo fields
 * carry storage keys the client already uploaded to the member-intake
 * bucket (mirrors saveTestResultLead in ../onboarding/actions.ts) — the
 * prefix check here is a sanity guard, not the real gate; that's the
 * bucket's RLS policy, enforced at upload time.
 */
export async function submitJoinData(
  ...
  if (!(PENGUNDANG_UNIT_OPTIONS as readonly string[]).includes(trimmed.pengundangUnit)) {
